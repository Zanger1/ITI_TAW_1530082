...
class EmpleadoInfonavitModel{
	...
	function __construct($IdEmpleadoInfonavit, $IdEmpleado, $MontoCreditoInfonavit, $MontoRetener, $MontoRestanteRetener, $FechaCaptura, $IdEmpleadoCaptura, $IdEmpleadoElimina, $FechaElimina, $Liquido, $Comentario, $Status){
	...
	public static function save($EmpleadoInfonavit){
		...
		$insert = $db->prepare('INSERT INTO empleadoinfonavit VALUES (NULL,:IdEmpleado,:MontoCreditoInfonavit,:MontoRetener,:MontoRestanteRetener,:FechaCaptura,:IdEmpleadoCaptura,null,null,:Liquido, :Comentario, :Status)');
	...
	public static function update($EmpleadoInfonavit){
		...
		$update=$db->prepare('UPDATE empleadoinfonavit SET IdEmpleado=:IdEmpleado, MontoCreditoInfonavit=:MontoCreditoInfonavit, MontoRetener=:MontoRetener, MontoRestanteRetener=:MontoRestanteRetener, Liquido=:Liquido, Comentario=:Comentario WHERE IdEmpleadoInfonavit=:IdEmpleadoInfonavit');
		#Hacer procedimientos almacenados
	...
	public static function all(){
		...
		foreach($select->fetchAll() as $EmpleadoInfonavit){$listaEmpleadoInfonavit[]=new EmpleadoInfonavitModel($EmpleadoInfonavit['IdEmpleadoInfonavit'],$EmpleadoInfonavit['IdEmpleado'],$EmpleadoInfonavit['MontoCreditoInfonavit'],$EmpleadoInfonavit['MontoRetener'],$EmpleadoInfonavit['MontoRestanteRetener'],$EmpleadoInfonavit['FechaCaptura'],$EmpleadoInfonavit['IdEmpleadoCaptura'],$EmpleadoInfonavit['IdEmpleadoElimina'],$EmpleadoInfonavit['FechaElimina'],$EmpleadoInfonavit['Liquido'], $EmpleadoInfonavit['Comentario'], $EmpleadoInfonavit['Status']);
		}
	...
	public static function get_total_all_records(){
		...
		$select=$db->prepare('SELECT e.*, ei.* FROM empleadoinfonavit AS ei, empleados AS e WHERE e.IdEmpleado = ei.IdEmpleado AND e.IdSucursal=:IdSucursal AND Liquido = 0 AND Status=0');
...
#		$filtro_busqueda=$_POST["search"]["value"];	//El $filtro de busqueda es el valor que el usuario escriba en el input. ref[1]
		...
		//$query .= 'SELECT e.*, ei.* FROM empleadoinfonavit AS ei, empleados AS e WHERE e.IdEmpleado = ei.IdEmpleado AND e.IdSucursal=:IdSucursal AND Liquido = 0 AND Status=0';
		$query .= 'SELECT e.*, ei.* FROM empleadoinfonavit AS ei, empleados AS e WHERE e.IdEmpleado = ei.IdEmpleado AND Liquido = 0 AND Status=0';
...
#		$statement->bindValue(':busqueda', $filtro_busqueda);	//Al param :busqueda se le asigna el $filtro_busqueda. ref[1],
	...
	public static function searchById($id){
		...
		$empleadosInfonavitDb['FechaCaptura'],$empleadosInfonavitDb['IdEmpleadoCaptura'],$empleadosInfonavitDb['IdEmpleadoElimina'],$empleadosInfonavitDb['FechaElimina'],$empleadosInfonavitDb['Liquido'],$empleadosInfonavitDb['Comentario'],$empleadosInfonavitDb['Status']);
		//var_dump($alumno);
		//die();
	...
	public static function delete($id, $idEE){
		...
		$delete=$db->prepare('UPDATE empleadoinfonavit SET IdEmpleadoElimina=:IdEmpleadoElimina, FechaElimina=:FechaElimina, Status=:Status WHERE IdEmpleadoInfonavit=:id');
