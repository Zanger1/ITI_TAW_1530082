...
//jlopezl
	...
	#		$insert=$db->prepare('INSERT INTO clientes VALUES (NULL,:Nombre,:RFC,:Calle,:Colonia,:CodigoPostal,:Num,:IdCiudad,:CorreoElectronico,:Telefono)');
		...
		$insert=$db->prepare('INSERT INTO unidades_renta VALUES (NULL, :DesUnidadRenta, 0)');
		...
		$insert->bindValue('DesUnidadRenta',$unidad->getDesUnidad());
		$insert->execute();
...
#			$insert->execute();
  ...
  //jlopez actualizo la funcion
	public static function all(){
		...
		$select=$db->query('CALL get_unidades_renta("")');
...
#		$filtro_busqueda=$_POST["search"]["value"];	//El $filtro de busqueda es el valor que el usuario escriba en el input. ref[1]
		...
		$query = "SELECT * FROM unidades_renta ";
####
		if( isset($search_filter) || isset($row) || isset($rowperpage) ){
			$query .= 'WHERE  DesUnidad LIKE "%'.$search_filter.'%" AND eliminado=0 ';
...
if(isset($_SESSION["id_role"])){
	...
	if($id_role == '3'){ //Solo para auxiliares
			$sub_array[] = '<button type="button" name="view" id="'.$row["IdUnidadRenta"].'" class="btn btn-success btn-sm view" data-toggle="tooltip" data-placement="top" title="Ver"><i class="fa fa-eye"></i></button> <button type="button" name="update" id="'.$row["IdUnidadRenta"].'" class="btn btn-info btn-sm update" data-toggle="tooltip" data-placement="top" title="Editar"><i class="fa fa-edit"></i></button> ';
	} else {	//Administradores: General y de sucursal
			$sub_array[] = '<button type="button" name="view" id="'.$row["IdUnidadRenta"].'" class="btn btn-success btn-sm view" data-toggle="tooltip" data-placement="top" title="Ver"><i class="fa fa-eye"></i></button> <button type="button" name="update" id="'.$row["IdUnidadRenta"].'" class="btn btn-info btn-sm update" data-toggle="tooltip" data-placement="top" title="Editar"><i class="fa fa-edit"></i></button> <button type="button" name="delete" id="'.$row["IdUnidadRenta"].'" class="btn btn-danger btn-sm delete" data-toggle="tooltip" data-placement="top" title="Eliminar"><i class="fa fa-trash"></i></button>';
	}
}
			$data[] = $sub_array;
		}
...
public static function searchById($id){
		$db=DataBase::getConnect();
		$select=$db->prepare('CALL get_unidades_renta_id(:id)');
		$select->bindValue('id',$id);
		$select->execute();
		$unidadesDb=$select->fetch();
		$unidades = new UnidadesRentaModel ($unidadesDb['IdUnidadRenta'],$unidadesDb['DesUnidad']);
		return $unidades;
	}
...
#		$select=$db->prepare('SELECT * FROM empleados WHERE IdEmpleado=:id');
		$select=$db->prepare('SELECT * FROM unidades_renta WHERE IdUnidadRenta=:id');
		$select->bindValue('id',$id);
		$select->execute();
		$unidad=$select->fetch();
		return $unidad['DesUnidad'];
	}



//jlopezl 
	public static function update($unidades){
		$db=DataBase::getConnect();
#		$update=$db->prepare('UPDATE clientes SET Nombre=:Nombre,    RFC=:RFC, Calle=:Calle, Colonia=:Colonia, CodigoPostal=:CodigoPostal, Num=:Num, IdCiudad=:IdCiudad, CorreoElectronico=:CorreoElectronico, Telefono=:Telefono WHERE IdCliente=:IdCliente');
			...
			$update=$db->prepare('CALL edit_unidades_renta(:IdUnidadRenta,:DesUnidad)');
			$update->bindValue('IdUnidadRenta',$unidades->getIdUnidadRenta());
			$update->bindValue('DesUnidad',$unidades->getDesUnidad());
			$update->execute();
...
/*
	...
	public static function delete($IdUnidadRenta){
		$db=DataBase::getConnect();
		$delete=$db->prepare('CALL delete_unidad_renta(:IdUnidadRenta)');
		$delete->bindValue('IdUnidadRenta',$IdUnidadRenta);
		$delete->execute();
	}
